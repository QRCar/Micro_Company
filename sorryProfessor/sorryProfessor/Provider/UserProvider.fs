...
module getUser =
    let getAllUser = dapperQuery<User> "Select id, login, firstname, lastname, school, promo, classe from master.[user]"
    
    let getUserById id = dapperMapParameterizedQuery<User>
                          "Select id, login, firstname, lastname, school, promo, classe from master.[user] where id = @UserId"
